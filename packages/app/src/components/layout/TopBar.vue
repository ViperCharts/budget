<template>
  <header class="h-14 md:h-16 flex items-center justify-between px-4 md:px-6 border-b border-[var(--color-border)] bg-[var(--color-surface)] shrink-0">
    <!-- Left: breadcrumb / page title -->
    <div class="flex items-center gap-3">
      <button
        @click="$emit('toggle-sidebar')"
        class="hidden md:flex lg:hidden btn-ghost p-2 rounded-lg"
      >
        <Menu class="w-5 h-5" />
      </button>
      <h1 class="font-heading font-semibold text-gray-900 dark:text-white text-base md:text-lg">
        {{ pageTitle }}
      </h1>
    </div>
    ...
    <div class="flex items-center gap-1.5 md:gap-2">
      <!-- AI tip (hidden on mobile) -->
      <div
        v-if="financialTip"
        class="hidden lg:flex items-center gap-2 max-w-sm px-3 py-1.5 rounded-lg bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300 text-xs font-body"
      >
      ...
      <!-- Global month picker -->
      <div class="flex items-center gap-0.5 border border-[var(--color-border)] rounded-lg px-0.5 md:px-1">
        <button class="btn-ghost p-1.5" title="Previous month" @click="monthStore.prevPeriod()">
          <ChevronLeft class="w-4 h-4" />
        </button>
        <span class="font-heading font-semibold text-xs md:text-sm text-gray-900 dark:text-white w-24 md:w-32 text-center select-none">
      ...
      <button @click="cycleTheme" class="hidden md:flex btn-ghost p-2">
      ...
      <!-- User avatar -->
      <div class="relative">
        <button @click="showUserMenu = !showUserMenu" class="flex items-center gap-1 md:gap-2 btn-ghost px-1.5 md:px-2 py-1.5">
          ...
          <ChevronDown class="w-3 h-3 text-gray-400 hidden md:block" />
        </button>
        ...
        >
          ...
          <button
            @click="cycleTheme"
            class="md:hidden w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
          >
            <Sun v-if="themeMode === 'light'" class="w-4 h-4" />
            <Moon v-else-if="themeMode === 'dark'" class="w-4 h-4" />
            <Monitor v-else class="w-4 h-4" />
